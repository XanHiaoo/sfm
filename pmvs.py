    ...
    def prepare_txt(self):
        ...
        for i in range(len(projections)):
            with open("./reconstruction/dense/pmvs/txt/" + str(i).zfill(8) + ".txt", "w") as f:
                f.write("CONTOUR\r"
                        + str(projections[i][0][0]) + " " + str(projections[i][0][1]) + " " + str(
                    projections[i][0][2]) + " " + str(projections[i][0][3]) + "\r"
                        + str(projections[i][1][0]) + " " + str(projections[i][1][1]) + " " + str(
                    projections[i][1][2]) + " " + str(projections[i][1][3]) + "\r"
                        + str(projections[i][2][0]) + " " + str(projections[i][2][1]) + " " + str(
                    projections[i][2][2]) + " " + str(projections[i][2][3]) + "\r")

    def save_model(self):
        if not os.path.exists(config.re_dir+'model'):
            os.makedirs(config.re_dir+'model')
        shutil.copyfile('./reconstruction/dense/pmvs/models/option-0000.ply', config.re_dir+'model/model.ply')
        print("点云保存成功！")
